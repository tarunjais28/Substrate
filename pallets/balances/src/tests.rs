...
macro_rules! decl_tests {
	($test:ty, $ext_builder:ty, $existential_deposit:expr) => {
		...
		use frame_support::{
			assert_noop, assert_ok, assert_err, StorageValue,
			traits::{
		...
		fn account_should_be_reaped() {
			<$ext_builder>::default().existential_deposit(1).monied(true).build().execute_with(|| {
				assert_eq!(Balances::free_balance(1), 10);
				assert_ok!(<Balances as Currency<_>>::transfer(&1, &0, 10, AllowDeath));
		...
		fn partial_locking_should_work() {
			<$ext_builder>::default().existential_deposit(1).monied(true).build().execute_with(|| {
				...
				assert_ok!(<Balances as Currency<_>>::transfer(&1, &0, 1, AllowDeath));
			});
		}
		...
		fn lock_reasons_should_work() {
			<$ext_builder>::default()
				...
				.execute_with(|| {
					...
					assert_noop!(
						<Balances as Currency<_>>::transfer(&1, &0, 1, AllowDeath),
						Error::<$test, _>::LiquidityRestrictions
					);
					assert_noop!(
		...
		fn lock_block_number_extension_should_work() {
			<$ext_builder>::default().existential_deposit(1).monied(true).build().execute_with(|| {
				...
				assert_noop!(
					<Balances as Currency<_>>::transfer(&1, &0, 6, AllowDeath),
					Error::<$test, _>::LiquidityRestrictions
				);
				...
				Balances::extend_lock(ID_1, &1, 10, WithdrawReasons::all());
				assert_noop!(
					<Balances as Currency<_>>::transfer(&1, &0, 3, AllowDeath),
					Error::<$test, _>::LiquidityRestrictions
				);
			});
		}
		...
		fn balance_transfer_works() {
			<$ext_builder>::default().build().execute_with(|| {
				...
				assert_ok!(Balances::transfer(Some(1).into(), 0, 69));
				assert_eq!(Balances::total_balance(&1), 42);
				assert_eq!(Balances::total_balance(&0), 69);
			});
		}
		...
		fn force_transfer_works() {
			<$ext_builder>::default().build().execute_with(|| {
				let _ = Balances::deposit_creating(&1, 111);
				assert_noop!(
					Balances::force_transfer(Some(0).into(), 1, 0, 69),
					BadOrigin,
				);
				assert_ok!(Balances::force_transfer(RawOrigin::Root.into(), 1, 0, 69));
				assert_eq!(Balances::total_balance(&1), 42);
				assert_eq!(Balances::total_balance(&0), 69);
			});
		}
		...
		fn balance_transfer_when_reserved_should_not_work() {
			<$ext_builder>::default().build().execute_with(|| {
				...
				assert_ok!(Balances::reserve(&1, 69));
				assert_noop!(
					Balances::transfer(Some(1).into(), 0, 69),
		...
		fn repatriating_reserved_balance_should_work() {
			<$ext_builder>::default().build().execute_with(|| {
				...
				let _ = Balances::deposit_creating(&0, 1);
				assert_ok!(Balances::reserve(&1, 110));
				assert_ok!(Balances::repatriate_reserved(&1, &0, 41, Status::Free), 0);
				assert_eq!(
					last_event(),
					Event::pallet_balances(RawEvent::ReserveRepatriated(1, 0, 41, Status::Free)),
				);
				assert_eq!(Balances::reserved_balance(1), 69);
				assert_eq!(Balances::free_balance(1), 0);
				assert_eq!(Balances::reserved_balance(0), 0);
				assert_eq!(Balances::free_balance(0), 42);
			});
		}
		...
		fn transferring_reserved_balance_should_work() {
			<$ext_builder>::default().build().execute_with(|| {
				...
				let _ = Balances::deposit_creating(&0, 1);
				assert_ok!(Balances::reserve(&1, 110));
				assert_ok!(Balances::repatriate_reserved(&1, &0, 41, Status::Reserved), 0);
				assert_eq!(Balances::reserved_balance(1), 69);
				assert_eq!(Balances::free_balance(1), 0);
				assert_eq!(Balances::reserved_balance(0), 41);
				assert_eq!(Balances::free_balance(0), 1);
			});
		}
		...
		fn transferring_incomplete_reserved_balance_should_work() {
			<$ext_builder>::default().build().execute_with(|| {
				...
				assert_ok!(Balances::repatriate_reserved(&1, &0, 69, Status::Free), 28);
				assert_eq!(Balances::reserved_balance(1), 0);
				assert_eq!(Balances::free_balance(1), 69);
				assert_eq!(Balances::reserved_balance(0), 0);
				assert_eq!(Balances::free_balance(0), 42);
		...
		fn transferring_too_high_value_should_not_panic() {
			<$ext_builder>::default().build().execute_with(|| {
				...
				Balances::make_free_balance_be(&0, 1);

				assert_err!(
					Balances::transfer(Some(1).into(), 0, u64::max_value()),
		...
		fn account_create_on_free_too_low_with_other() {
			<$ext_builder>::default().existential_deposit(100).build().execute_with(|| {
				...
				// No-op.
				let _ = Balances::deposit_creating(&0, 50);
		...
		fn transfer_keep_alive_works() {
			<$ext_builder>::default().existential_deposit(1).build().execute_with(|| {
				let _ = Balances::deposit_creating(&1, 100);
				assert_noop!(
					Balances::transfer_keep_alive(Some(1).into(), 0, 100),
					Error::<$test, _>::KeepAlive
				);
				assert_eq!(Balances::total_balance(&1), 100);
				assert_eq!(Balances::total_balance(&0), 0);
			});
		}
